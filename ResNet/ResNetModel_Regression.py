...
from sklearn.metrics import precision_recall_curve, auc, confusion_matrix, precision_score, recall_score, classification_report
...
total_phenotypes = 8
is_volcani = True
labels = ["necrosis", "Burning", "Bleaching", "Chlorosis", "Epinasty_curling", "Inhibited_growth", "Wilting", "Disturbed"]
...
def get_params(device):
    ...
    return {
        ...
        'size': (512, 256),
        'loss_mask': torch.FloatTensor([1, 1, 1, 1, 1, 1, 1, 1]).to(device),
        'batch_size': 2,
        'depth': 1,
        'train_fraction': 0.7,
        'test_fraction': 0.5,
...
def load_data(df_train, df_val, df_test, params, device):
    ...
    test_dataset = CustomDataSet(resized_path, resized_img_path, data_path, data_path_img, tfms=tfms,  # params['tfms']
...
def train_model(train_loader, test_loader, params, optimizer, net):
    ...
    early_stopping = EarlyStopping(patience=params['patience'], verbose=True, delta=params['delta'])
    for i in range(params['epochs']):
        ...
        for b, (X_train, y_train) in enumerate(train_loader):
            ...
            loss = cal_loss(y_pred, y_train.float())
            # backprop
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if b % 20 == 0:
                print(f'epoch: {i + 1:2},   batch: {b_num}   loss: {scaled_loss:10.8f}')
        ...
        test_maes.append(average_test_epoch_mae)
    ...
    expirement_df = expirement_df.append({'date': date.today(), 'epochs': params['epochs'], 'stride': params['stride_first_layer'], 'dimension_reduction': params['dimension_reduction'], 'learning_rate': params['learning_rate'], 'decay': params['weight_decay'], 'depth': params['depth'], 'val_loss': test_losses, 'train_loss': train_losses, 'val_mae': test_maes,  'val_mae_element': test_maes_element, 'duration': duration, 'batch_size': params['batch_size']}, ignore_index=True)
...
def cal_loss_mse(input, target):
    ...
    loss = loss * params['loss_mask'].reshape((1, 8))
    loss = torch.sum(loss)
    return loss
...
def cal_loss(input, target):
    ...
    loss = element_loss * params['loss_mask'].reshape((1, 8))
...
def valuate_model(test_loader_val, net, params):
    ...
    total_predictions = []
    total_labels = []
    test_running_mae_element = torch.zeros(8).to(device)
    with torch.no_grad():
        b_num_test = 0
    ...
    # over all eval
    total_predictions = torch.flatten(total_predictions)
    total_labels = torch.flatten(total_labels)
    ...
    return average_test_loss, average_test_mae
...
def gpu_checks():
    ...
    labels_df = pd.read_csv(f"{results_path}/phenotyping_with_groups_sparse.csv")
...
if __name__ == '__main__':
    ...
    if only_resize:
        ...
        n_classes = 8  # todo:df_train["label"][0].shape[0]
        ...
        # valuate on test
        average_test_loss, average_test_mae = valuate_model(val_loader, net, params)
        # # save model
