...
is_resize = False
only_resize = False
total_img = -1  # if no limit set to -1
is_volcani = True
...
def get_params(device):
    ...
    """
    parameters = {
        'dimension_reduction': 10,
        'dimension_span': 2,
        'stride_first_layer': 2,
        'size': (512, 256),
        'loss_mask': torch.tensor([1., 1., 1., 1., 1., 1., 1., 1.]).to(device),   # [1., 0., 1., 0., 1., 1., 1., 1.]
        'batch_size': 1,
        'depth': 1,
        'train_fraction': 0.7,
        'test_fraction': 0.5,
        ...
        'BCE_weights': torch.tensor([[0.62, 0.38], [0.8, 0.2], [0.58, 0.42], [0.8, 0.2], [0.48, 0.52], [0.72, 0.28],
                                     [0.74, 0.26], [0.5, 0.5]]).to(device),
        # 'BCE_weights': torch.tensor([[0.5, 0.5], [0.5, 0.5], [0.5, 0.5], [0.5, 0.5], [0.5, 0.5], [0.5, 0.5],
        #                              [0.5, 0.5], [0.5, 0.5]]).to(device),
        'BCE_param': 0.5,
...
class ResidualBlock(nn.Module):
    """
    basic structure add residual(input) to output after going threw blocks
    """
...
class ResNetResidualBlock(ResidualBlock):
    """
    define the shortcut that conv2d to match channels to block output by expanding.
    applyed only if input is different then expanded channels
    """
...
def conv_bn(in_channels, out_channels, conv, *args, **kwargs):
    """stacks a conv and batchnormalazation"""
...
class ResNetBottleNeckBlock(ResNetResidualBlock):
    ...
    the authors defined a BottleNeck block that “The three layers are 1x1, 3x3, and 1x1 convolutions,
...
class ResNetLayer(nn.Module):
    ...
    first block changes channels from in_channels to out_channels
    All other block keep out_channels (if expansion = 1).
    If expansion != 1, then each block expands to out_channels * block.expansion,
    which will be the input channel for next block
    """
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, in_channels=3, blocks_sizes=[64, 128, 256, 512], deepths=[2, 2, 2, 2],
                 activation='relu', block=ResNetBasicBlock, stride_first_layer=2, *args, **kwargs):
        super().__init__()
        self.blocks_sizes = blocks_sizes

        self.gate = nn.Sequential(
            nn.Conv2d(in_channels, self.blocks_sizes[0], kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(self.blocks_sizes[0]),
            activation_func(activation),
            nn.MaxPool2d(kernel_size=stride_first_layer, stride=stride_first_layer)
        )
...
def get_layers_channels(in_channel, layers, dimension_span):
    block_channels = []
    for layer in range(layers):
        if layer == 0:
            out_channels = in_channel * dimension_span
        else:
            out_channels = block_channels[layer - 1] * dimension_span
        block_channels.append(int(out_channels))
    return block_channels


def init_model(params, input_channel, n_classes, device):
    gate_out_channels = input_channel // params["dimension_reduction"]
    layer_channels = get_layers_channels(gate_out_channels, params['layer_n'], params["dimension_span"])
    ResNetModel = ResNet(input_channel, n_classes, block=ResNetBasicBlock, blocks_sizes=layer_channels,
                         deepths=[2, 2, 2, 2, 2], stride_first_layer=params['stride_first_layer'])
...
def cal_loss_BCE_old(input, target, weight, batch_size):
    ...
    # take only masked phenotypes
    loss_class_weighted = loss_class_weighted.reshape([batch_size, -1])
    # indices = torch.nonzero(params['loss_mask']).flatten()
    # loss_class_weighted = loss_class_weighted[:, indices]
    loss_class_weighted = loss_class_weighted.mean()
    return loss_class_weighted


def cal_loss_BCE2(input, target, weight, batch_size):
    indices = target.data.view(-1).long()
    indices = indices.unsqueeze(-1)
    weight_ = torch.gather(weight, 1, indices)
    criterion = nn.BCELoss(reduce=False)
...
def cal_loss_BCE_23_05(input, target, weight, batch_size):
    ...
    # loss = loss * params['loss_mask'].reshape((1, params['loss_mask'].shape[0]))
    # loss = torch.reshape(loss, (-1, 1)) * weight
    # loss = loss.reshape([batch_size, -1])input, target
    # indices = torch.nonzero(params['loss_mask']).flatten()
    # loss = loss[:, indices]
    loss = loss.mean()
    return loss
...
def valuate_model(test_loader_val, net, params):
    ...
    for col in range(total_predictions.shape[1]):
        ...
        cm_per_phenotype = plot_confusion_matrics(total_predictions[:, col], total_labels[:, col], labels_name_filtered[col], labels_ranking)
        acc_per_phenotype[labels_name_filtered[col]] = accuracy_score(total_predictions[:, col].cpu(), total_labels[:, col].cpu(), normalize=True)
        plot_recall_precision(total_labels[:, col].cpu(), total_scores[:, col].cpu(), labels_name_filtered[col] + "_prior=" + str(prior[col]))
...
def gpu_checks():
    ...
    labels_df = pd.read_csv(f"{results_path}/phenotyping_with_groups_sparse.csv") # phenotyping_with_groups_2.csv
    ...
    if only_resize:
        ...
        if is_train:
            ...
            net = ResNet(input_channel, params["dimension_reduction"], params["stride_first_layer"],
                                 params["dimension_span"], total_phenotypes, params['dropout_p'])
