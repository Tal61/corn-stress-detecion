...
class ResNet(nn.Module):
    def __init__(self, in_channels, dimension_reduction, stride, dimension_span, n_classes, dropout_p):
        super().__init__()
        ...
        self.out1 = int(self.in_channels // self.dimension_reduction)
...
def init_model(params, input_channel, n_classes, device):
    ResNetModel = ResNet(input_channel, params["dimension_reduction"], params["stride_first_layer"],
                         params["dimension_span"], n_classes, params['dropout_p'])
...
if __name__ == '__main__':
    ...
    if only_resize:
        ...
        if is_train:
            # init model
            net, criterion, optimizer = init_model(params, input_channel, total_phenotypes, device)
